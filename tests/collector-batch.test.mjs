...
const context = {
  ...
  PropertiesService: {
    getScriptProperties: () => ({ getProperty: (key) => key === "STUDY_VERSION" ? "2026-08-25-v7" : "" })
  }
};
vm.runInNewContext(source, context, { filename: "Code.gs" });

const identity = { participantId: "P001", studyId: "STUDY01", sessionId: "S001" };
const allocationId = "n30-three-versions-v1-slot-04";
const participantRow = {
  ...
  assignment_version: "n30-three-versions-v1",
  status: "in_progress",
  completed_trials: 49,
...
function trialPayload(globalTrialIndex, eventId) {
  ...
  return {
    ...
    resumeState: { version: "2026-08-25-v7", status: "in_progress", trialCursor: globalTrialIndex },
    record: {
      eventId,
      participantSlot: 4,
      allocationId,
      ...
      randomizationSeed: `text-enrichment-reader-n30-v2:participant:4:documents:set:${setId}:rotation:${(setId - 1) * 13}`,
      rating: 2,
      spatialRating: -2,
      responseTime: 840,
